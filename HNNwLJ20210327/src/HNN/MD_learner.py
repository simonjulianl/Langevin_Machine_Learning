...
class MD_learner:
    ...
    def __init__(self, linear_integrator_obj, any_HNN_obj, phase_space, init_path, load_path, crash_filename=None):
        ...
        self._dataset_obj = dataset(init_path)

        print("============ train/valid data ===============")
        self.q_train, self.p_train = self._dataset_obj.qp_dataset('train', crash_filename)
        self.q_valid, self.p_valid = self._dataset_obj.qp_dataset('valid')
        ...
        self.q_train_label, self.p_train_label = self._dataset_obj.phase_space2label(self.q_train, self.p_train, self.linear_integrator, self._phase_space, self.noML_hamiltonian)
        self.q_valid_label, self.p_valid_label = self._dataset_obj.phase_space2label(self.q_valid, self.p_valid, self.linear_integrator, self._phase_space, self.noML_hamiltonian)
