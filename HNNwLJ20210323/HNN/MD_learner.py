...
class MD_learner:
    ...
    def __init__(self, linear_integrator_obj, any_HNN_obj, phase_space, init_path, crash_filename=None):
        ...
        print("============ data loaded ===============")
        self.train_data, self.tau_train = self._data_io_obj.qp_dataset('train', crash_filename)
        # self.train_data = self.train_data[:4]  # check debug
        # print('n. of data', self.train_data)
        print('n. of data', self.train_data.shape, self.tau_train.shape)

        self.valid_data, self.tau_valid = self._data_io_obj.qp_dataset('valid')
        # self.valid_data = self.valid_data[:2]  # check debug
        # print('n. of data', self.valid_data)
        print('n. of data', self.valid_data.shape, self.tau_valid.shape)

        print("============ data label ===============")
        # qnp x iterations x nsamples x  nparticle x DIM
        self.train_label = self._data_io_obj.phase_space2label(self.train_data, self.tau_train, self.linear_integrator, self._phase_space, self.noML_hamiltonian)
        self.valid_label = self._data_io_obj.phase_space2label(self.valid_data, self.tau_valid, self.linear_integrator, self._phase_space, self.noML_hamiltonian)
    ...
    def train_valid_epoch(self, save_path, best_model_path, loss_curve):
        ...
        self._tau_cur = self._data_io_obj.diff_tau_list(self._q_train, MD_parameters.tau_long_list)
        MD_iterations = int(self.tau_train[0] / self._tau_cur[0] )
        ...
        for e in range(1, ML_parameters.nepoch + 1 ):
            ...
            for i in range(random_ordered_train_nsamples): # load each sample for loop
                ...
                q_train_batch, p_train_batch, tau_train_batch = self._q_train[i], self._p_train[i], self.tau_train[i] # each sample
                ...
                tau_train_batch = torch.unsqueeze(tau_train_batch, dim=0).to(self._device)
                ...
                q_train_pred, p_train_pred = self.linear_integrator.step( self.any_HNN, self._phase_space, MD_iterations, nsamples_cur, tau_train_batch)
            ...
            with torch.no_grad():
                ...
                for j in range(random_ordered_valid_nsamples):
                    ...
                    q_valid_batch, p_valid_batch, tau_valid_batch = self._q_valid[j], self._p_valid[j], self.tau_valid[j]
                    ...
                    tau_valid_batch = torch.unsqueeze(tau_valid_batch, dim=0).to(self._device)
                    ...
                    q_valid_pred, p_valid_pred = self.linear_integrator.step( self.any_HNN, self._phase_space, MD_iterations, nsamples_cur, tau_valid_batch)
