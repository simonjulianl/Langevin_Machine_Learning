...
class data_io:
    ...
    def qp_dataset(self, filename, crash_filename = None, shuffle = True):
        ...
        print('shape', q_list1.shape, p_list1.shape)

        tau_long_list = self.diff_tau_list(q_list1, MD_parameters.tau_long_list)
        ...
        return init, tau_long_list

    def diff_tau_list(self, q_list, tau_list):

        ''' function to prepare different tau about n. of data ( train or valid or test )

        Parameters
        ----------
        q_list : train or valid or test
        num : multiply of q_list

        Returns
        ----------
        tau_list : torch.tensor
                different time steps, shape is n. of data

        '''

        tau_list = torch.tensor(tau_list)
        num = len(q_list) // tau_list.shape[0]

        assert (len(q_list) == num * tau_list.shape[0]), "n. of q and tau not match error"

        tau_list = tau_list.repeat(num)
        return tau_list

    def phase_space2label(self, qp_list, tau_list, linear_integrator, phase_space, noML_hamiltonian):
        ...
        tau_cur : torch.tensor
                tau list at short time step for label
        MD_iterations : int
                n iterations at short time step
        Returns
        ----------
        ...
        tau_cur = self.diff_tau_list(curr_q, MD_parameters.tau_short_list)
        MD_iterations = int(tau_list[0] / tau_cur[0] )
        ...
        for z in range(0, len(curr_q), nsamples_batch):
            ...
            linear_integrator.step( noML_hamiltonian, phase_space, MD_iterations, nsamples_batch, tau_cur[z:z+nsamples_batch])
            q_list[:, z:z + nsamples_batch], p_list[:, z:z + nsamples_batch] = linear_integrator.concat_step(MD_iterations, tau_cur[z:z+nsamples_batch])
