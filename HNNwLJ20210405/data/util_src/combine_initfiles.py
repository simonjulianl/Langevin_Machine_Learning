...
def qp_list_combine(filename):
    ...
    qp_list_app = torch.cat(qp_list_app, dim=2) # along nsamples
...
if __name__ == '__main__':
    ...
    filename = sorted(glob.glob(root_path + 'run*/' + 'nparticle{}_new_nsim_rho0.1_T'.format(nparticle) + '*' + '_{}_sampled.pt'.format(mode)), key=keyFunc)
    write_filename = root_path + 'n{}_new_nsim_rho0.1_allT_{}_sampled.pt'.format(nparticle, mode)
