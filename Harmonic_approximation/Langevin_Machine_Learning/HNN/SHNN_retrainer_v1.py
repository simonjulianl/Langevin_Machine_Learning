...
class SHNN_retrainer:
    ...
    def __init__(self, level, folder_name, **kwargs):
        ...
        **kwargs : trainer_setting, containing : 
        ...
        try : #data loader and seed setting
            ...
            self._optimizer = self._checkpoint['optimizer']
            print('Previously trained optimizer state_dict loaded...')
            # load the criterion
            self._loss = self._checkpoint['loss']
            print('Trained model loss function loaded...')
            self._n_epochs = self._checkpoint['epoch']
            print(f"Previously trained for {self._n_epochs} number of epochs...")
            # train for more epochs
            self._n_epochs = int(kwargs['new_epoch'])
            print(f"Train for {self._n_epochs} more epochs...")
            #scheduler
            self._scheduler = self._checkpoint['scheduler']

            self._best_validation_loss = self._checkpoint['best_validation_loss']

            self._curr_level = self._checkpoint['level']

            self._batch_size = self._checkpoint['batch_size'] # will be used for data loader setting
        ...
        # to log all the data 
        self._writer = SummaryWriter('runs/samples{}_tau{}/{}_retrain'.format(self._sample,self._time_step,folder_name)) # explicitly mention in runs folder
    ...
    def train_epoch(self):
        ...
        for batch_idx, (data,label) in enumerate(self._train_loader) : 
            ...
            if self._scheduler :# if scheduler exists
                ...
                print('Epoch {}, lr {}'.format(self._n_epochs, self._optimizer.param_groups[0]['lr']))
    ...
    def record_best(self, validation_loss, filename = 'checkpoint.pth') :
        ...
        if is_best :
            ...
            shutil.copyfile(filename, './retrain_saved_model/samples{}_tau{}_batch{}_lr{}_model_best.pth'.format(self._sample,self._time_step,self._batch_size,self._optimizer.param_groups[0]['lr']))
    ...
    def train(self):
        ...
        for i in range(self._level_epochs):
            ...
            self.train_level(filename = './retrain_saved_model/samples{}_tau{}_batch{}_lr{}_checkpoint_level{}.pth'.format(self._sample,self._time_step,self._batch_size,self._optimizer.param_groups[0]['lr'],self._curr_level))
