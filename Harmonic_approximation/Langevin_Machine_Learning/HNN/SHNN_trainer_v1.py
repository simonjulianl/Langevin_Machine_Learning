...
class SHNN_trainer: 
    ...
    def validate_epoch(self, validation_loader):
        ...
        diff = 0
        diff_2 = 0 # to calculate variance and hence std
        for batch_idx, (data,label) in enumerate(validation_loader) :
            ...
            #print('label',label[0])
            del_q = torch.pow((prediction[0] - label[0]),2)
            del_p = torch.pow((prediction[1] - label[1]),2)
            #print('del_q',del_q)
            #print(del_q.shape)
            #print('del_p',del_p)
            #print(del_p.shape)
            del_q_batch = torch.sum(del_q,dim=0) # batch sum
            del_p_batch = torch.sum(del_p,dim=0) # batch sum
            #print(del_q_batch)
            #print(del_p_batch)
            del_q_batch = torch.sum(del_q_batch,dim=-1) # coordinate sum
            del_p_batch = torch.sum(del_p_batch,dim=-1) # coordinate sum
            #print(del_q_batch)
            #print(del_p_batch)
            #print(del_q_batch.shape[0])
            del_q_batch_particle = torch.sum(del_q_batch) / del_q_batch.shape[0]
            del_p_batch_particle = torch.sum(del_p_batch) / del_p_batch.shape[0]
            #print(del_q_batch_particle)
            #print(del_p_batch_particle)
            #print(del_q_batch_particle+del_p_batch_particle)
            diff_qp = del_q_batch_particle.item() + del_p_batch_particle.item()
            diff += diff_qp

            # del_q2 = torch.pow((prediction[0] - label[0]),2)**2.0  #del_q*del_q
            # del_p2 = torch.pow((prediction[1] - label[1]),2)**2.0
            #
            # del_q2_batch = torch.sum(del_q2,dim=0)
            # del_p2_batch = torch.sum(del_p2, dim=0)
            #
            # del_q2_batch = torch.sum(del_q2_batch,dim=-1) # coordinate sum
            # del_p2_batch = torch.sum(del_p2_batch,dim=-1) # coordinate sum
            #
            # del_q2_batch_particle = torch.sum(del_q2_batch) / del_q2_batch.shape[0]
            # del_p2_batch_particle = torch.sum(del_p2_batch) / del_p2_batch.shape[0]
            #
            # diff_2 += del_q2_batch_particle.item() +  del_p2_batch_particle.item()

            diff_2 += diff_qp * diff_qp
            print('diff_qp',diff_qp)
            print('diff', diff)
            print('diff_qp2', diff_qp*diff_qp)
            print('diff_2',diff_2)
            validation_loss += loss.item() # get the scalar output

        print('valid_dataset len',len(validation_loader.dataset))
        diff /= len(validation_loader.dataset)
        diff_2 /= len(validation_loader.dataset)
        print('epoch',diff)
        print('epoch', diff*diff)
        print('epoch',diff_2)

        return (diff, validation_loss / len(validation_loader.dataset), diff_2) #return the average

    def record_best(self, validation_loss, diff, diff_2, filename = 'checkpoint.pth') :
        '''
    ...
    def train_level(self, filename = 'checkpoint.pth'):
        ...
        if self._curr_level != 1 : 
            ...
            diff, validation_loss, diff_2 = self.validate_epoch(self._test_loader)
            print('performance of level {} weight on level {}'.format(self._curr_level - 1, self._curr_level))
            std_qp = (diff_2 - diff ** 2.0) ** 0.5
            
            print('\t diff : {:.5f} ± {:.5f} \n\t test loss : {:.6f}'.format(
                diff, std_qp, validation_loss
                ))
            
        for i in range(1, self._n_epochs + 1):
            train_loss = self.train_epoch()
            diff, validation_loss, diff_2 = self.validate_epoch(self._validation_loader)
            #print('diff_2', diff_2)
            #print('diff*diff', diff** 2.0)
            print('epoch:{} train_loss:{:.6f} ; validation_loss.{:.6f}'.format(i,train_loss,validation_loss))

            self.record_best(validation_loss, diff, diff_2, filename)
            self._current_epoch += 1
            ...
            self._writer.add_scalar('qp difference_level {}'.format(self._curr_level),
                        diff,
                        i # epoch
                        )
        ...
        #print('diff_2',self._best_state['diff_2'])
        #print('diff*diff',self._best_state['diff'] ** 2.0)
        std_qp = (self._best_state['diff_2']- self._best_state['diff'] ** 2.0) ** 0.5
        #print(std_qp)
        print('\t qp_diff : {} ± {} '.format(self._best_state['diff'],std_qp))
