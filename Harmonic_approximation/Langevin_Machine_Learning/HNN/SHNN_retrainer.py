...
class SHNN_retrainer:
    ...
    def __init__(self, level, folder_name, **kwargs):
        ...
        **kwargs : trainer_setting, containing : 
        ...
        try : #data loader and seed setting
            ...
            #self._optimizer.load_state_dict(self._checkpoint['optimizer'].state_dict()) # when save optimizer not optimizer.state_dict()
    ...
    def record_best(self, validation_loss, filename = 'checkpoint.pth') :
        ...
        if is_best :
            ...
            shutil.copyfile(filename, './retrain_{}_saved_model/samples{}_tau{}_batch{}_lr{}_model_best.pth'.format(self._retrain_num,self._sample,self._time_step,self._batch_size,self._optimizer.param_groups[0]['lr']))
    ...
    def train(self):
        ...
        if not os.path.exists('./retrain_{}_saved_model/'.format(self._retrain_num)):
            os.makedirs('./retrain_{}_saved_model/'.format(self._retrain_num))

        for i in range(self._level_epochs):
            print('current level : {}'.format(self._curr_level))
            self.train_level(filename = './retrain_{}_saved_model/samples{}_tau{}_batch{}_lr{}_checkpoint_level{}.pth'.format(self._retrain_num,self._sample,self._time_step,self._batch_size,self._optimizer.param_groups[0]['lr'],self._curr_level))
