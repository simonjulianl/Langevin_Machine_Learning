    ...
    def integrate(self,multicpu=True):
        ...
        if not multicpu:
            ...
            def integrate_helper(num, return_dict , **state):
                ...
                **state : dict
                ...
                for i in trange(total_samples):
                    ...
                    #print('Langevin.py p',p)
                    p = np.exp(-gamma * time_step / 2) * p + np.sqrt(  kB * Temp / m * ( 1 - np.exp( -gamma * time_step))) * random_1[i][num:num+total_particle]
                    #print('Langevin.py p random_1',random_1[i][num:num+total_particle])
                    #print('Langevin.py p random1', p)
                ...
                #print('Langevin.py return',np.array(return_dict[num]).shape)
                #print('Langevin.py return', np.array(return_dict[num]))
            ...
            for i in return_dict.keys(): #skip every 1000
                ...
                #print('Langevin.py return_dict[i][0]', return_dict[i][0])
                #print('Langevin.py return_dict[i][1]', return_dict[i][1])
