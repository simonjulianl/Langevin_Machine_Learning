...
class Integration(ABC) : 
    ...
    def __init__(self, *args, **kwargs) -> object:           
        ...
        **kwargs : 
        ...
        if pos is None :  # create random particle initialization of U[-1,1) for q (pos) and v if not supplied
            ...
            pos = np.array([[0.2,0.2],[-0.2,-0.2]])
            #pos = np.array([[0.2*2, 0.3*2], [-0.3*2, 0.2*2], [0.1*2, -0.1*2], [-0.1*2, -0.2*2]])
            #print('base_simulation.py pos',pos)
        ...
        if vel is None :
            ...
            #
            # vel = np.array(vel)
            vel = np.array([[0.01,0.03],[-0.01,-0.02]])
    ...
    def set_phase_space(self, samples : int = -1) :
        ...
        filename = '/N_particle{}_samples50000_rho0.2_T{}_pos_sampled.npy'.format(particle, temp)
