...
class MD_learner:
    ...
    def __init__(self, linear_integrator_obj, any_HNN_obj, phase_space, dataset, load_path):
        ...
        self.q_train = dataset[0][0]; self.p_train = dataset[0][1]
        self.q_valid = dataset[1][0]; self.p_valid = dataset[1][1]

        self.q_train_label = dataset[2][0]; self.p_train_label = dataset[2][1]
        self.q_valid_label = dataset[3][0]; self.p_valid_label = dataset[3][1]
    ...
    def valid_epoch(self,  niter, nsamples_cur):
        ...
        for j in range(0, self.no_valid_nsamples, nsamples_cur):
            ...
            q_valid_label_batch, p_valid_label_batch = self.q_valid_label[:, j:j + nsamples_cur], self.p_valid_label[:,
                                                                                                  j:j + nsamples_cur]
    ...
    def nepoch(self, niter, save_path, best_model_path, save_loss_curve):
