...
class metropolis_mc:
    ...
    def step(self, hamiltonian, phase_space):
        ...
        new mcs     : the number of samples for mc

        Returns
        ___________
        q_list      : shape is nsample X nparticle X DIM
        U           : potential energy
        AccRatio    : acceptance rate to update new energy
        spec        : estimate specific heat from fluctuations of the potential energy

       '''

        niter = MC_parameters.iterations - MC_parameters.DISCARD
        q_list = torch.zeros((MC_parameters.new_mcs, niter, MC_parameters.nparticle, MC_parameters.DIM), dtype = torch.float64)
        U = torch.zeros(MC_parameters.new_mcs, niter)
