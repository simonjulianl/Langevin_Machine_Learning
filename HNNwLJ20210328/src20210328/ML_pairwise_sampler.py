...
from HNN.data_preprocess        import data_preprocess
...
data_preprocess_obj = data_preprocess( linear_integrator_obj, init_path)
dataset_obj = dataset()

print("============ train/valid data ===============")
qp_train_list = data_preprocess_obj.qp_list_combine( init_filename, 'train')
qp_valid_list = data_preprocess_obj.qp_list_combine(init_filename, 'valid')

qp_train = dataset_obj.qp_dataset(qp_train_list)
qp_valid = dataset_obj.qp_dataset(qp_valid_list)

print("============ label for train/valid ===============")
qp_train_label = data_preprocess_obj.phase_space2label(qp_train, noMLhamiltonian, phase_space, tau_short, niter_tau_short, append_strike)
qp_valid_label = data_preprocess_obj.phase_space2label(qp_valid, noMLhamiltonian, phase_space, tau_short, niter_tau_short, append_strike)

dataset = [(qp_train, qp_valid), (qp_train_label, qp_valid_label)]

# print(torch.all(torch.eq(qp_valid[1],dataset[0][1][1])))
