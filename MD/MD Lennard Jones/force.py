...
force_double_basin2D(1,2,1,np.array([[1,2]]),scale = True)
        
            
def electrostatic(N,DIM,q,BoxSize,pos,kappa,nk):
    pot_r = ewald_module.pot_r_ewald(pos,q,kappa,BoxSize)
    pot_k = ewald_module.pot_k_ewald(nk,pos,q,kappa,BoxSize)
    #dipole = np.sum(q[:,None] * r[:,:],axis = 0)
#    pot_s = (2 * np.pi / 3) * np.sum(dipole ** 2) # surface potential
    #total pot = pot_r + pot_k + pot_s
    return pot_r + pot_k # pot k already included self potential
