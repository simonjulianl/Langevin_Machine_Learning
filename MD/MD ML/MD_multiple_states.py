...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  6 12:18:38 2020
...
''' here the sequence of data used are only used to predict the next output without needing each of 
the sequence to predict the next step '''

parser = argparse.ArgumentParser('MD Multiple States FC')
args = parser.add_argument('--log_interval', type = int, default = 1000, help = 'log interval for loss')
args = parser.add_argument('--total_time', type = float ,default = 101, help = 'Total N steps', metavar = 'N')
...
alpha = args.alpha
INITIAL_DIM = 10 #all the history in the p and q positions are used as input
...
dataset_train = create_dataset(mode = 'train' , total_time_slow = total_time, slow_time_step = deltat, LSTM = False)
kwargs = {'pin_memory' : True, 'num_workers' : 6, 'shuffle' : True}
train_loader = DataLoader(dataset_train, batch_size = batch_size, **kwargs)

dataset_validation = create_dataset(mode = 'validation', total_time_slow = total_time, slow_time_step = deltat, LSTM = False)
valid_loader = DataLoader(dataset_validation, batch_size = batch_size , **kwargs)

dataset_test = create_dataset(mode = 'test', total_time_slow = total_time, slow_time_step = deltat, LSTM = False)
...
def train(epoch):
    ...
    for batch_idx, (data, label_sequence) in enumerate(train_loader):
        ...
        predicted_state = model(data)
        initial_state = data[:,0] #we take the initial position as reference for energy
