...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 23 15:46:56 2020
...
args = parser.add_argument('--epochs', type = int, default = 10, help = 'Number of epochs')
...
class MDModel(nn.Module):
    def __init__(self):
        ...
        self.conv1d = nn.Sequential(
            nn.Conv1d(1,40, kernel_size = 1,stride = 1),
    ...
    def forward(self,x):
        ...
        return self.fc1(x_pos), self.fc2(x_pos) #fc2 is the energy 
...
def combined_loss(initial_energy, initial_state, final_state):
    return (1 + alpha) * loss_function_pos_vel(initial_state, final_state) + (1 - alpha) * loss_function_energy(initial_energy, final_state)
    
model = MDModel().to(device)
# summary(model,input_size = (1,2))
optimizer = optim.AdamW(model.parameters(), lr = 1e-3, weight_decay = 0.1) #adam with weight decay
...
def original_error():
    ...
    with torch.no_grad():
        for batch_idx, (data,true_state) in enumerate(train_loader):
            data = data.unsqueeze(dim = 1).to(device)
            true_state = true_state.to(device)
            loss = loss_function_pos_vel(data, true_state)
