...
class phase_space :
    ...
    def set_p(self, p_list):
        print('phase_space.py p_list', p_list.shape)
        self._p_list = p_list 
    
    def set_q(self, q_list):
        print('phase_space.py q_list',q_list.shape)
        self._q_list = q_list 
    
    def get_p(self):
        print('phase_space.py _p_list', self._p_list.shape)
        return self._p_list # N X particle X DIM array
    
    def get_q(self):
        print('phase_space.py _q_list', self._q_list.shape)
        return self._q_list # N X particle X DIM array
    ...
    def read(self, filename, samples = -1):
        ...
        self._p_list = np.array(self._q_list[:samples])
