...
class LJ_term:
    ...
    def phi_fields(self, xi_space, grids_list): # HK

        '''
        phi_fields function to get phi field each grid
        ...
        # xi_state.shape = [nsamples, nparticle, DIM = (x,y)]
        boxsize = xi_space.get_boxsize()

        nsamples, nparticle, DIM = xi_state.shape
        gridL2, DIM = grids_list.shape   # gridL2 = gridL * gridL

        grids_state = grids_list.unsqueeze(1)
        # grids_state.shape is [gridL * gridL, 1, DIM=2] = [gridL2, 1, DIM]

        xi_state = xi_state.expand((gridL2, nsamples, nparticle, DIM))
        # xi_state.shape is [ gridL * gridL, nsamples, nparticle, DIM]

        a12 = (4 * self._epsilon * pow(self._sigma, 12)) / pow(boxsize, 12)
        a6 = (4 * self._epsilon * pow(self._sigma, 6)) / pow(boxsize, 6)

        fields = torch.zeros((xi_state.shape[1], grids_state.shape[0]))
        # fields.shape is [ nsamples, npixels ]

        for z in range(nsamples):

            grids_xi = torch.cat((grids_state, xi_state[:,z]),dim=1)
            # shape is [gridL * gridL, nparticle+1, DIM ]

            _, d = xi_space.paired_distance_reduced(grids_xi, nparticle+1, DIM) # concat one pixel so that nparticle+1
            # d is distance btw grid and xi
            # d.shape is  [ gridL * gridL, nparticle+1 , DIM]

            s12 = 1 / pow(d, 12)
            s6 = 1 / pow(d, 6)

            fields_dim = a12 * torch.sum(s12, dim=-1) - a6 * torch.sum(s6, dim=-1)
            # fields_dim.shape is [ gridL * gridL, nparticle+1]

            fields[z] = fields_dim.sum(dim=-1)
            # fields[z].shape is [ gridL * gridL ]
