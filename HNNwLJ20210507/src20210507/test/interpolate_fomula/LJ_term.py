...
class LJ_term:
    ...
    def phi_fields(self, xi_space, grids_list): # HK
        ...
        _, gridL2, DIM = grids_list.shape
        # grids_list.shape = [nsamples, gridL2 = gridL * gridL, DIM = (x,y)]
        ...
        xi_state = xi_state.permute(1,0,2,3)
        # shape is [nsamples, gridL * gridL, nparticle, DIM ]

        grids_list = grids_list.unsqueeze(dim=2)
        grids_xi = torch.cat((grids_list, xi_state),dim=2)
        # shape is [nsamples, gridL * gridL, nparticle+1, DIM ] # HK
        print('grids_xi', grids_xi.shape)
        quit()
