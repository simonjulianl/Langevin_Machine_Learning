...
class pairwise_HNN(hamiltonian):
    ...
    def __init__(self, pairwise_MLP1, pairwise_MLP2):
        super(pairwise_HNN,self).__init__()  # SJ
    ...
    def dHdq_all(self, phase_space, net):
        ...
        #x = torch.zeros([nsamples*nparticle*nparticle,5],dtype=torch.float64)
        ...
        #predict2 = torch.zeros(predict2.shape)

        corrected_dHdq = noML_dHdq + predict2
        #corrected_dHdq = noML_dHdq  # HK

        print('memory total:', psutil.virtual_memory()[0], '\n') #total
        print('memory used:', psutil.virtual_memory()[3]/1e6, '\n') #used
        print('memory percent:', psutil.virtual_memory()[2], '\n') # percent
