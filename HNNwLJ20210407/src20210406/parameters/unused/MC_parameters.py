...
class MC_parameters:
    ...
    nparticle = 2
    temperature = 0.04
    mode = 'test'                              # set mode train or valid or test for filename

    init_path           = '../data/init_config/n{}run13/'.format(nparticle)
    filename            = init_path + 'n{}_nsim_rho0.1T{}_{}_sampled.pt'.format(nparticle, temperature, mode)

    seed = 15343                                # set different seed for generate data for train/valid/test
    ...
    dq = 0.005                                  # displacement to increase acceptance rate
