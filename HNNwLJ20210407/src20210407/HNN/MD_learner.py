...
class MD_learner:
    ...
    def __init__(self, linear_integrator_obj, any_HNN_obj, phase_space, opt, data_loader):
        ...
        chk_pt      :  checkpoint obj for save or load them
        '''
        ...
        self._opt = opt
        self.chk_pt = checkpoint(ML_parameters.chk_pt_path, self.any_HNN.netA, self.any_HNN.netB, self._opt)

        if ML_parameters.chk_pt_file is not None:
            self.chk_pt.load_checkpoint(ML_parameters.chk_pt_file)
    ...
    # ===================================================
    def nepoch(self, write_chk_pt_filename, write_loss_filename):
        ...
        for e in range(1, ML_parameters.nepoch + 1 ):
            ...
            print(train_set_len, self.batch_size)
            train_loss_avg = train_loss / ( train_set_len // self.batch_size )
            # valid_loss_avg = valid_loss / ( valid_set_len // ML_parameters.batch_size )
