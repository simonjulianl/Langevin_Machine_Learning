...
class DoubleConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('cat cnn pbc x',x.shape)
        x = self.double_conv_first(x)
        # print('cat cov first x',x.shape)
        x = compute_PBC(x, self.PBC_constant)
        # print('cat cnn pbc x',x.shape)
        out = self.double_conv_second(x)
        # print('cat cov second',out.shape)
        return out
...
class FC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        cat_ntau = ML_parameters.cat_ntau

        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2))

        self.fully_connected = nn.Sequential(
            nn.Linear(in_channels + cat_ntau, out_channels),
            nn.Tanh(),
