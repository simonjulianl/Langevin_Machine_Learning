...
class pair_wise_HNN:
    ...
    def phase_space2data(self, phase_space, pb):
        ...
        for z in range(N):
            delta_init_q_, _ = pb.paired_distance_reduced(q_list[z] / self._state['BoxSize'], N_particle,
                                                          DIM)  # reduce distance for pb
            delta_init_q_ = delta_init_q_ * self._state['BoxSize']  # not reduce distance

            delta_init_p_, _ = pb.paired_distance_reduced(p_list[z] / self._state['BoxSize'], N_particle,
                                                          DIM)  # reduce distance for pb
        ...
        # print('tau', tau.shape)
        paired_data_ = torch.cat((delta_init_q, delta_init_p),
                                 dim=-1)  # N (nsamples) x N_particle x (N_particle-1) x (del_qx, del_qy, del_px, del_py)
        # print(paired_data_)
        # print(paired_data_.shape)
        paired_data = torch.cat((paired_data_, tau),
                                dim=-1)  # nsamples x N_particle x (N_particle-1) x  (del_qx, del_qy, del_px, del_py, tau )
        paired_data = paired_data.reshape(-1, paired_data.shape[
            3])  # (nsamples x N_particle) x (N_particle-1) x  (del_qx, del_qy, del_px, del_py, tau )
