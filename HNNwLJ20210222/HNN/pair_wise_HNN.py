...
class pair_wise_HNN(hamiltonian):
    ...
    def phase_space2data(self, phase_space, tau_cur):
        ...
        for z in range(0, nsamples, nsamples_batch):

            delta_init_q_ = self.delta_qorp(q_list[z:z+nsamples_batch], nparticle, DIM)
            delta_init_p_ = self.delta_qorp(p_list[z:z+nsamples_batch], nparticle, DIM)

            delta_init_q[z:z+nsamples_batch] = delta_init_q_
            delta_init_p[z:z+nsamples_batch] = delta_init_p_

        # tau : #this is big time step to be trained
        # to add paired data array, reshape
        tau = torch.tensor([tau_cur] * nsamples_batch * nparticle * (nparticle - 1)).to(ML_parameters.device)
        ...
        paired_data = paired_data.reshape(-1, paired_data.shape[-1])  # (nsamples x nparticle) x (nparticle-1) x  (del_qx, del_qy, del_px, del_py, tau )
