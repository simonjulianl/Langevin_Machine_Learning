...
class pair_wise_MLP(nn.Module):
    ...
    def forward(self, data, nparticle, DIM): # data -> del_list ( del_qx, del_qy, del_px, del_py, t )
        ...
        MLdHdq_ = MLdHdq_.reshape(MD_parameters.nsamples_batch, nparticle, nparticle - 1, DIM)  # N_particle, N_particle-1, DIM
        MLdHdq = torch.sum(MLdHdq_, dim=2) # ex) a,b,c three particles;  sum Fa = Fab + Fac
